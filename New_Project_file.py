...
def show_results_page():
    ...
    new_df = df2.groupby('Sector').agg({'Total Seats': 'sum', 'Sold Seats': 'sum', 'Unsold Seats': 'sum', 'MAT_Ratio': 'mean', 'Release Ratio': 'mean'}).reset_index()
    ...
    st.button("Home",on_click=lambda: set_page('upload'))
    
def set_page(page):
    st.session_state.page = page
    st.rerun()
    # if st.button("Home"):
        # st.session_state.page = 'upload'  # Navigate back to the upload page
         # Force rerun to update the page and avoid double clicking
# Function to show the P/L Report page
...
def show_pl_analytics_page():
    st.markdown("<h2 style='text-align: center;'>P/L Analytics</h2>", unsafe_allow_html=True)
    ...
    fig = px.pie(new_df2, values='Profit/Loss', names='Sector', title='Contribution of each sector to total Profit/Loss')
